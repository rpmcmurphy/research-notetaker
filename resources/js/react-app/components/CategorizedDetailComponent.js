...
function CategorizedDetailComponent() {
    ...
    useEffect(() => {
        ...
        const fetchDetail = async () => {
            try {
                const response = await detailApi.getDetailsByTopic(topicId, currentPage);
                                    ...
                                    <Col md={3} key={`detail_${index}`} className="mb-2">
                                        <Card style={{ width: '100%', height: '100%' }}>
                                            ...
                                            <Card.Body style={{ display: 'flex', flexDirection: 'column' }}>
                                                <Card.Title style={{fontSize: '16px'}}>{detail.details_name }</Card.Title>
...
export default CategorizedDetailComponent;